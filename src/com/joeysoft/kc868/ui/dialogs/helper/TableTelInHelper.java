...
public final class TableTelInHelper {
	 ...
	 * 刷新Table
	 */
	public void showTable(Table table, List<TelIn> list){
		table.removeAll();
		for(TelIn telIn : list){
			insertTable(table, telIn);
		}
	}
	 ...
	 * @param telIn
	 */
	public void insertTable(Table table, TelIn telIn){
		TableItem item = new TableItem(table, SWT.NONE); // 创建Item
		item.setData(telIn);
	    item.setText(new String[] {""+table.getItemCount(), telIn.getTelName(), telIn.getCountryCode(), telIn.getTelPhone()});// 给Item设值
	}
	 ...
	 * @param floor
	 */
	public void updateTable(Table table, TelIn telIn){
		if(telIn != null){
			TableItem[] tia = table.getSelection();
			tia[0].setData(telIn);
			tia[0].setText(new String[] {tia[0].getText(0), telIn.getTelName(), telIn.getCountryCode(), telIn.getTelPhone()});// 给Item设值
		}
	}
